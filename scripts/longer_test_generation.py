"""
Longer test generation - runs HeyGen test with a longer script.
"""
...
def main():
    """Generate longer test output."""
    safe_print("="*60)
    safe_print("Longer Test Generation")
    safe_print("="*60)
    safe_print("\nThis will generate a HeyGen video with a longer script.")
    ...
    # Test script path (longer script)
    script_path = project_root / "examples" / "scripts" / "longer_test_script.txt"
    ...
    # Output directory
    output_dir = project_root / "examples" / "outputs" / "longer_tests"
    output_dir.mkdir(parents=True, exist_ok=True)
    
    safe_print(f"\nOutput directory: {output_dir}")
    safe_print(f"Test script: {script_path}")
    safe_print(f"\nStarting longer test generation...\n")
    time.sleep(1)
    ...
    # Append to existing log file (don't delete it - we have multiple tests running)
    ...
    try:
        ...
        with open(log_file, 'a', encoding='utf-8') as f:
            f.write(f"[{start_time}] [START] Longer test generation started\n")
            f.flush()
    except Exception:
        pass
    
    # Generate output filename
    output_name = "HeyGen_Longer_Test.mp4"
    ...
    try:
        ...
        error_msg = f"\n[X] Error generating longer test: {e}"
